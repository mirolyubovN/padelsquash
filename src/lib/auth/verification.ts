...
export async function issueEmailVerificationCode(args: {
	...
	const verifyUrl = `${getAppBaseUrl()}/register/verify?email=${encodeURIComponent(targetEmail)}${args.nextPath.startsWith("/") ? `&next=${encodeURIComponent(args.nextPath)}` : ""
		}`;

	const sent = await sendEmailMessage({
		to: targetEmail,
		subject: "Код подтверждения email для Racket Community Kst",
		text: [
...
export async function issueEmailVerification(args: {
	...
	const sent = await sendEmailMessage({
		...
		subject: "Подтвердите email для Racket Community Kst",
		text: [
			`Здравствуйте, ${args.name}.`,
			"",
			"Подтвердите email, чтобы завершить регистрацию в Racket Community Kst:",
			verificationUrl,
			"",
