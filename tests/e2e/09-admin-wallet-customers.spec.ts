...
test("admin can create a customer from wallet page and prefill balance adjustment", async ({ page }) => {
  ...
  await row.getByRole("link", { name: "Пополнить баланс" }).click();
  await expect(page.locator("#wallet-customer-email")).toHaveValue(email);

  await page.locator("#wallet-amount-kzt").fill("20000");
  await page.locator("#wallet-note").fill("Наличная оплата в клубе");
