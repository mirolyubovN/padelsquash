...
export const metadata = buildPageMetadata({
	title: "Забронировать | Racket Community Kst",
...
type CourtPriceMatrix = Record<string, Record<"off_peak" | "peak", number>>;
...
async function getCourtPriceMatrix(locationId: string): Promise<CourtPriceMatrix> {
	...
	try {
		...
		if (rows.length > 0) {
			for (const row of rows as Array<{
				...
				if (!base[sportSlug]) {
					base[sportSlug] = { off_peak: 0, peak: 0 };
				}
				base[sportSlug][row.period as "off_peak" | "peak"] = Number(row.amount);
			}
			return base;
		}
	} catch {
	...
	for (const item of demoComponentPrices) {
		if (item.componentType === "court" && item.currency === "KZT") {
			if (!base[item.sport]) {
				base[item.sport] = { off_peak: 0, peak: 0 };
			}
			base[item.sport][item.tier as "off_peak" | "peak"] = item.amount;
		}
	}
	return base;
}
