...
export const metadata = buildPageMetadata({
	title: "Цены | Racket Community Kst",
...
export default async function PricesPage() {
	...
	const publicPriceMatrixRows = courtRows.map((row) => ({
		item: row.label,
		offPeak: `${row.values.off_peak.toLocaleString("ru-KZ")} ₸`,
		peak: `${row.values.peak.toLocaleString("ru-KZ")} ₸`,
	}));
	...
	const exampleCourtPeak = exampleCourtRow?.values.peak ?? 0;
	...
	const exampleInstructorMin = exampleTrainerPrices.length
		? [...exampleTrainerPrices].sort((a, b) => a - b)[0]
		: null;
	const exampleTotal = exampleInstructorMin !== null ? exampleCourtPeak + exampleInstructorMin : null;

	const sportEntries = Array.from(
		new Map(courtRows.map((row) => [row.sport, row.sportName])).entries(),
	);
					...
					<div className="pricing-example__card">
						...
						<div className="pricing-example__rows">
							<div className="pricing-example__row">
								<span>Корт ({exampleSportName.toLowerCase()}), пиковое время</span>
								<strong>{exampleCourtPeak.toLocaleString("ru-KZ")} ₸</strong>
							</div>
							<div className="pricing-example__row">
								<span>Тренер ({exampleSportName.toLowerCase()}), от</span>
								<strong>
									{exampleInstructorMin !== null
										? `${exampleInstructorMin.toLocaleString("ru-KZ")} ₸`
